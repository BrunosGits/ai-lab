...
- **#17561** Inaccurate codec error message: Root cause is a hardcoded list in the error message, fix derives the message from the accepted codec list (built-ins + registered Lucene codecs + aliases, deduped + sorted). Two tests cover it, e2e verified via `EngineConfigTests`. **PR #22749** resolved its merge conflict (rebase onto latest main, kept both the codec tests and upstream's new toBuilder test in `EngineConfigTests`), ran green on the VPS, and is now **mergeable + approved**, only waiting on gradle-check.
- **#22706** Flaky `AnalyticsQueryTaskCleanupIT`: root cause is the test injecting a raw `TaskCancelledException` on the streaming channel where production sends `StreamException(StreamErrorCode.CANCELLED)`. Fix is a one line change. **PR #22750** currently still mixes both this fix and the codec fix from #17561; the split (drop the codec commit so #22750 carries only this fix) is the next step, branch not yet re-pushed.
