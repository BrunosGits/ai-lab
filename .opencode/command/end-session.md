...
6. If today is a new journal day, refresh the total time in the right-aligned line at the top of `journal.md` using the tracker totals.
7. Write the prose under the user's humanizer rules: no semicolons, no em or en dashes, no AI-writing tells, first person, and never mention the Mac. The fields only contain the user's own facts. Do not invent any. Let the user read and approve the journal text before moving on.
8. Commit with a message like `docs: journal, <summary>` and push to origin main.
