...
### Sync Forks
```bash
cd /home/bruno/ai-lab/testcontainers-java && git fetch upstream && git merge upstream/main
cd /home/bruno/ai-lab/testcontainers-rs && git fetch upstream && git merge upstream/main
```
...
# Java
cd /home/bruno/ai-lab/testcontainers-java
git checkout -b fix/9876-debug-logging
git add -A && git commit -m "fix: avoid getDockerImageName() in debug logging when disabled
Closes #9876"
git push origin fix/9876-debug-logging
gh pr create --title "fix: lazy debug logging for GenericContainer" --body "..."

# Rust
cd /home/bruno/ai-lab/testcontainers-rs
git checkout -b fix/926-standard-label
git add -A && git commit -m "feat: add org.testcontainers=true label for ecosystem consistency
Closes #926"
git push origin fix/926-standard-label
gh pr create --title "feat: add standard org.testcontainers=true label" --body "..."
```